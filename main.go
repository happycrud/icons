		...
		if category.IsDir() {
			...
			for _, svg := range svgs {
				...
				for _, t := range shap {
					...
					x := bytes.ReplaceAll(p, []byte(`height="24"`), []byte(`height={ size } width={ size }`))
					x = bytes.ReplaceAll(x, []byte(`width="24"`), []byte(``))
					x = bytes.ReplaceAll(x, []byte(`viewBox="0 0 24 24"`), []byte(`viewBox={ "0 0 " + size + " " + size } class={classes}`))
					x = bytes.ReplaceAll(x, []byte(`enable-background="new 0 0 24 24"`), []byte(`enable-background={ "new 0 0 " + size + " " + size }`))
